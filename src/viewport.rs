...
use crate::{sprite::Sprite, get_graphics, graphics::Renderable, GRAPHICS};

pub type ViewportType = Weak<RwLock<Viewport>>;
...
impl Viewport {
	pub fn new(x: f32, y: f32, width: f32, height: f32) -> Arc<RwLock<Self>> {
		...
		match GRAPHICS.get() {
			...
			None => {println!("[Viewport] This viewport has been created on window initialization")}	
		}
