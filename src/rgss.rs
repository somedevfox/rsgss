...
use magnus::{eval, value::Qnil, RClass, RString, RArray, RModule};
	...
	thread::spawn(move || {
		...
		if !Path::new("Data/xScripts.rxdata").exists() {
			MessageDialog::new()
				...
				.set_description("Game Scripts do not exist! (File Not Found: Data/xScripts.rxdata)")
		...
		let file_result: RString = file.funcall("binread", ("Data/xScripts.rxdata",)).unwrap();
		
		// Unmarshal them
		let marshal = RModule::from_value(eval("Marshal").unwrap()).unwrap();
