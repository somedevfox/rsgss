...
pub struct Graphics {
	...
	pub viewports: RwLock<Vec<ViewportType>>
}
impl Graphics {
	pub async fn create_window(
		...
		let (device, queue) = adapter.request_device(
			&wgpu::DeviceDescriptor {
				...
				limits: if cfg!(target_arch = "wasm32") { // rsgss wasm32 support one day maybe?
		...
		let window_viewport = Viewport::new(
			0.0, 
			0.0, 
			width as f32, 
			height as f32
	...
	pub fn update(&self) {
		for viewport in &*self.viewports.read() {
